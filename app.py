...
# Config
# ---------------------------------------------------------------------------
MODEL = "gemini-flash-latest"  # Alias que Google mantiene apuntando al Flash vigente (gratis, sin tarjeta)
...
if "history" not in st.session_state:
    st.session_state.history = []  # list of {"role": "user"/"model", "parts": [text]}
...
def get_ai_turn(model, user_text: str):
    ...
    st.session_state.history.append({"role": "model", "parts": [raw]})
...
with st.sidebar:
    ...
    api_key = st.text_input(
        ...
        type="password",
        help="Consíguela gratis en aistudio.google.com (solo necesitas una cuenta de Google, sin tarjeta). "
             "No se guarda en ningún lado, solo se usa en esta sesión.",
    )
...
if not api_key:
    st.warning(
        "Ingresa tu Google AI Studio API key en la barra lateral para empezar. "
